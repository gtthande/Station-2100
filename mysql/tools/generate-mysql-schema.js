...
function convertColumnType(pgType, maxLength = null, isPrimary = false) {
  let mysqlType = TYPE_MAPPING[pgType.toLowerCase()] || 'TEXT';
  
  // Handle specific cases
  if (pgType.toLowerCase().includes('varchar') && maxLength) {
    mysqlType = `VARCHAR(${Math.min(maxLength, 65535)})`;
  } else if (pgType.toLowerCase().includes('character varying') && maxLength) {
    mysqlType = `VARCHAR(${Math.min(maxLength, 65535)})`;
  } else if (pgType.toLowerCase().includes('character') && maxLength) {
    mysqlType = `CHAR(${maxLength})`;
  }
  
  // Handle UUID primary keys
  if (isPrimary && pgType.toLowerCase() === 'uuid') {
...
// Generate MySQL schema based on Station-2100 structure
function generateMySQLSchema() {
  const schema = `-- Station-2100 MySQL Shadow Database Schema
...
function generateAndSaveSchema() {
  ...
  console.log(`✅ MySQL schema generated: ${schemaPath}`);
  return schemaPath;
}
...
module.exports = { generateMySQLSchema, generateAndSaveSchema };
