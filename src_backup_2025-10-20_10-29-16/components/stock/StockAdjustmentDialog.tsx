...
import { useStockMovements, StockMovementEvent } from '@/hooks/useStockMovements';
import {
  Dialog,
  DialogContent,
  DialogHeader,
...
export function StockAdjustmentDialog({ open, onOpenChange }: StockAdjustmentDialogProps) {
  ...
  const onSubmit = async (data: AdjustmentForm) => {
    const adjustmentType = data.adjustment_type as StockMovementEvent;
    const quantity = adjustmentType === 'ADJUSTMENT_OUT' ? -data.quantity : data.quantity;
    const sourceRef = generateSourceRef(adjustmentType, { productId: data.product_id });
          ...
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>
                      Quantity {adjustmentType === 'ADJUSTMENT_OUT' ? '(Remove)' : '(Add)'}
                    </FormLabel>
                    <FormControl>
                ...
                <FormItem>
                  ...
                  <FormControl>
                    <Textarea 
                      {...field} 
                      rows={3} 
                      placeholder="Describe the reason for this stock adjustment..."
                    />
                  </FormControl>
                  <FormMessage />
                </FormItem>
            ...
            <div className="flex gap-3 justify-end">
              ...
              <Button type="submit" disabled={isCreatingMovement}>
                {isCreatingMovement ? 'Processing...' : 'Process Adjustment'}
              </Button>
            </div>
          </form>
        </Form>
