...
interface EditInstallationLogDialogProps {
  logId: string;
  open: boolean;
  onOpenChange: (open: boolean) => void;
}

export const EditInstallationLogDialog = ({ logId, open, onOpenChange }: EditInstallationLogDialogProps) => {
  const { logs, updateLog, isUpdating } = useInstallationRemovalLogs();
  ...
  useEffect(() => {
    if (log) {
      form.reset({
        rotable_part_id: log.rotable_part_id,
        aircraft_id: log.aircraft_id,
        log_type: log.log_type,
        log_date: log.log_date,
        flight_hours_at_action: log.flight_hours_at_action,
        flight_cycles_at_action: log.flight_cycles_at_action,
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="bg-surface-dark border-white/10 text-white max-w-2xl">
        <DialogHeader>
          <DialogTitle className="text-xl font-semibold">Edit Installation/Removal Log</DialogTitle>
        </DialogHeader>
