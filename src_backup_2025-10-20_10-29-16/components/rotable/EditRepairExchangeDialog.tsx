...
interface EditRepairExchangeDialogProps {
  recordId: string;
  open: boolean;
  onOpenChange: (open: boolean) => void;
}

export const EditRepairExchangeDialog = ({ recordId, open, onOpenChange }: EditRepairExchangeDialogProps) => {
  const { records, updateRecord, isUpdating } = useRepairExchange();
  ...
  useEffect(() => {
    if (record) {
      form.reset({
        rotable_part_id: record.rotable_part_id,
        record_type: record.record_type,
        sent_to_facility: record.sent_to_facility,
        ...
        warranty_terms: record.warranty_terms || '',
        new_tso_hours: record.new_tso_hours,
        new_tso_cycles: record.new_tso_cycles,
        exchange_part_serial: record.exchange_part_serial || '',
        work_order_number: record.work_order_number || '',
        certification_reference: record.certification_reference || '',
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="bg-surface-dark border-white/10 text-white max-w-3xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="text-xl font-semibold">Edit Repair/Exchange Record</DialogTitle>
        </DialogHeader>
