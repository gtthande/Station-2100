# ================== STATION-2100 | Cursor one-paste setup (Vite Version) ==================
$ErrorActionPreference = "Stop"

# 0) Install deps needed for the guard (safe if already installed)
...
$dirs = @(
  "src/lib/supabase",
  "src/lib/auth",
  "src/components/admin/dev-tools",
  ".github/workflows"
)
...
'@ | Set-Content -Encoding UTF8 -Path "src/lib/supabase/client.ts"
...
'@ | Set-Content -Encoding UTF8 -Path "src/lib/auth/useAdminGuard.ts"

# 4) API route: latest commit from GitHub (Vite proxy approach)
@'
// Add this to your vite.config.ts in the server.proxy section:
// proxy: {
//   "/api/github": {
...
//           proxyReq.setHeader("Authorization", `Bearer ${import.meta.env.VITE_GITHUB_TOKEN}`);
//         }
//       });
//     }
//   }
// }
'@ | Set-Content -Encoding UTF8 -Path "src/lib/github-proxy-setup.md"

# 5) Admin Dev Tools component
@'
"use client";
...
export default function DevToolsClient() {
  ...
  useEffect(() => {
    (async () => {
      try {
        // Using GitHub API directly since we're in Vite
...
'@ | Set-Content -Encoding UTF8 -Path "src/components/admin/dev-tools/DevToolsClient.tsx"

# 6) Admin Dev Tools page wrapper
@'
...
'@ | Set-Content -Encoding UTF8 -Path "src/components/admin/dev-tools/index.tsx"

# 7) (Optional) GitHub Action (manual dispatch build)
@'
name: Build
on:
  workflow_dispatch:
...
# 8) Update vite.config.ts with GitHub proxy (optional)
Write-Host "`n📝 Optional: Add GitHub proxy to vite.config.ts:"
Write-Host "Add this to your server.proxy section:"
Write-Host "proxy: {"
Write-Host "  '/api/github': {"
Write-Host "    target: 'https://api.github.com',"
Write-Host "    changeOrigin: true,"
Write-Host "    rewrite: (path) => path.replace(/^/api/github/, ''),"
Write-Host "  }"
Write-Host "}"

# 9) Remind about env vars
Write-Host "`nAdd to .env (set your values):"
Write-Host "VITE_GITHUB_REPO=gtthande/Station-2100"
Write-Host "# VITE_GITHUB_TOKEN=ghp_xxx   # (optional for public repos)"

Write-Host "`n✅ Files created. Now commit & push:"
Write-Host "   git-sync-up `"Add Admin Dev Sync Panel + GitHub integration`""
# ======================================================================
