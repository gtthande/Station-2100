...
export const BatchApprovalReport = () => {
  ...
  const { getFullCompanyInfo } = useCompanyDetails();
  ...
  const handleExportToExcel = () => {
    ...
    const companyInfo = getFullCompanyInfo();
    ...
    const headerData = [
      ...
      { 'Generated By': user?.email || 'Unknown User' },
      { '': '' },
      { 'BATCH DATA': '' },
      { '': '' }
    ];
    ...
    // Create batch data
    const batchData = pendingBatches.map(batch => ({
    ...
    const allData = [...headerData, ...batchData];

    const worksheet = XLSX.utils.json_to_sheet(allData);
    
    // Set column widths
    worksheet['!cols'] = [
      { wch: 20 }, // Company Name column
      { wch: 30 }  // Value column
    ];

    const workbook = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(workbook, worksheet, 'Batch Approval Report');
