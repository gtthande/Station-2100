...
-- Create a simplified trigger to log profile changes
...
BEGIN
    IF TG_OP = 'UPDATE' THEN
        ...
        IF OLD.position IS DISTINCT FROM NEW.position THEN
            changed_fields := changed_fields || jsonb_build_object('position', jsonb_build_object('old', OLD.position, 'new', NEW.position));
        END IF;
        
        -- Log the changes if any fields were modified
