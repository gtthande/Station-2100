...
CREATE POLICY IF NOT EXISTS "Users can view their own custom roles" 
ON public.custom_roles 
FOR SELECT 
USING (auth.uid() = user_id);

CREATE POLICY IF NOT EXISTS "Users can create their own custom roles" 
ON public.custom_roles 
FOR INSERT 
WITH CHECK (auth.uid() = user_id);

CREATE POLICY IF NOT EXISTS "Users can update their own custom roles" 
ON public.custom_roles 
FOR UPDATE 
USING (auth.uid() = user_id);

CREATE POLICY IF NOT EXISTS "Users can delete their own custom roles" 
ON public.custom_roles 
FOR DELETE 
USING (auth.uid() = user_id);

-- Create RLS policies for user_custom_roles
CREATE POLICY IF NOT EXISTS "Users can view their own custom role assignments" 
ON public.user_custom_roles 
FOR SELECT 
USING (auth.uid() = user_id);

CREATE POLICY IF NOT EXISTS "Users can assign custom roles to themselves" 
ON public.user_custom_roles 
FOR INSERT 
WITH CHECK (auth.uid() = user_id);

CREATE POLICY IF NOT EXISTS "Users can update their own custom role assignments" 
ON public.user_custom_roles 
FOR UPDATE 
USING (auth.uid() = user_id);

CREATE POLICY IF NOT EXISTS "Users can delete their own custom role assignments" 
ON public.user_custom_roles 
FOR DELETE 
USING (auth.uid() = user_id);

-- Create update trigger for custom_roles
CREATE TRIGGER IF NOT EXISTS update_custom_roles_updated_at
  BEFORE UPDATE ON public.custom_roles
  FOR EACH ROW
  EXECUTE FUNCTION public.update_updated_at_column();

-- Create update trigger for user_custom_roles
CREATE TRIGGER IF NOT EXISTS update_user_custom_roles_updated_at
