-- CRITICAL SECURITY FIX: Secure Employee Personal Information (Corrected)
-- This migration addresses the critical security vulnerability in the profiles table

-- Step 1: Add security_admin role to app_role enum
...
CREATE TABLE IF NOT EXISTS public.secure_employee_credentials (
    ...
    -- Store hashed/encrypted sensitive data only
    pin_hash text, -- Store hashed PIN, never plain text
    biometric_template_hash text, -- Store hashed biometric template, never raw data
...
-- Create extremely restrictive RLS policies for secure credentials
CREATE POLICY "Only security admins can access credentials"
ON public.secure_employee_credentials
FOR ALL
TO authenticated
USING (
    -- Only allow access to admin or security_admin roles
...
-- Remove the extremely dangerous sample_password field
...
-- Step 4: Create comprehensive profile access logging
...
-- Enable RLS on security log - only admins can see logs
...
BEGIN
    ...
    -- Check if access should be granted based on existing can_view_profile function
    SELECT public.can_view_profile(_profile_id) INTO access_granted;
    
    -- Get client IP (handle potential null)
    BEGIN
    ...
    -- Log every access attempt with detailed information
    INSERT INTO public.profile_security_log (
        ...
        _access_type, 
        access_granted,
        CASE WHEN NOT access_granted THEN 'Insufficient permissions or unauthorized access attempt' ELSE NULL END,
        current_ip,
...
-- Step 6: Replace existing profile RLS policies with secure ones
...
-- New ultra-secure policies with mandatory logging
CREATE POLICY "Secure profile access with comprehensive logging"
ON public.profiles
FOR SELECT
TO authenticated
USING (
...
-- Allow profile creation but with logging
...
WITH CHECK (
    auth.uid() = id AND
    public.secure_profile_access(id, 'view') -- This will log the creation
);

-- Prevent profile deletion (maintain audit trail)
...
SELECT 
    ...
    p.staff_active,
    p.created_at,
    -- Conditional access to sensitive PII
    CASE 
        WHEN public.has_role(auth.uid(), 'admin'::public.app_role) 
             ...
             OR auth.uid() = p.id
        THEN p.email 
        ELSE '[PROTECTED]' -- Show that field exists but is protected
    END as email,
    CASE 
        WHEN public.has_role(auth.uid(), 'admin'::public.app_role) 
             OR public.has_role(auth.uid(), 'hr'::public.app_role)
        ...
        ELSE '[PROTECTED]'
    END as phone,
    -- Show badge_id only to security personnel
    CASE 
        WHEN public.has_role(auth.uid(), 'admin'::public.app_role) 
             OR public.has_role(auth.uid(), 'security_admin'::public.app_role)
        THEN p.badge_id
...
-- Step 9: Security hardening - revoke unnecessary permissions
