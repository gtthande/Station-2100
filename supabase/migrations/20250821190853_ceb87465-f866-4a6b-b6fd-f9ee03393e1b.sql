-- Fix Customer Contact Information Security Issues (Final)
...
-- 2. Drop and recreate RLS policies for the main customers table
DROP POLICY IF EXISTS "Admin and manager only customer access" ON public.customers;
DROP POLICY IF EXISTS "Admin-only customer deletion" ON public.customers;

-- Create new restrictive policies
CREATE POLICY "Admin and manager customer access" ON public.customers
FOR SELECT
USING (
...
CREATE POLICY "Admin and manager customer modification" ON public.customers
FOR INSERT
WITH CHECK (
  auth.uid() = user_id AND (
    has_role(auth.uid(), 'admin'::app_role) OR 
...
CREATE POLICY "Admin and manager customer updates" ON public.customers
FOR UPDATE
USING (
  auth.uid() = user_id AND (
    has_role(auth.uid(), 'admin'::app_role) OR 
...
CREATE POLICY "Admin only customer deletion" ON public.customers
FOR DELETE
USING (
